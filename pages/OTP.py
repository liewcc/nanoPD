...
if "otp_start_input" not in st.session_state:
    st.session_state["otp_start_input"] = "0x40130000"
if "otp_end_input" not in st.session_state:
    st.session_state["otp_end_input"] = "0x40130FFF"
...
# 5. Pre-fetch State
is_scanning = st.session_state.get("otp_scanning", False)
mounted = is_mounted()
...
with col1:
    with st.container(border=True):
        ...
        OTP_MODES = {
            ...
            "RAW (No ECC)":     0x40134000,
            "GUARDED":          0x40138000,
            "ECC BITS":         0x4013c000
        }

        selected_mode = st.selectbox("Read Mode", options=list(OTP_MODES.keys()), index=0, disabled=mounted or is_scanning)
        mode_base = OTP_MODES[selected_mode]
        
        # Mode-to-Address Auto-Sync logic
        if st.session_state.get("otp_mode_prev") != selected_mode:
            st.session_state["otp_start_input"] = hex(mode_base)
            st.session_state["otp_end_input"] = hex(mode_base + 0xFFF)
            st.session_state["otp_mode_prev"] = selected_mode
            st.rerun()

        start_addr_str = st.text_input("Start Address (Hex)", key="otp_start_input", disabled=mounted or is_scanning)
        end_addr_str   = st.text_input("End Address (Hex)",   key="otp_end_input",   disabled=mounted or is_scanning)
        
        st.markdown("<div style='margin-top: 10px;'></div>", unsafe_allow_html=True)
        
        button_label = "⏳ SCANNING..." if is_scanning else "SCAN OTP"
...
if is_scanning and not mounted:
    try:
        s_addr = int(st.session_state.get("otp_start_input", "0x40130000"), 16)
        e_addr = int(st.session_state.get("otp_end_input",   "0x40130FFF"), 16)
        ...
        if s_size > 16384: # Safety cap for OTP
            ...
            if res.returncode == 0:
                ...
                st.session_state["otp_data"] = "Scan Error: " + res.stderr.decode()
